...
* Uncle Dave Emacs
   参考： [[https://www.youtube.com/watch?v=d6iY_1aMzeg&list=PLX2044Ew-UVVv31a0-Qn3dA6Sd_-NyA1n][Uncle Dave Emacs]]
...
*** TODO export chinese pdf ?
   ...
   to open pdf files with zathura. setting ~org-file-apps~ .
...
#+end_src
   ...
   + *Caution!* Must write strictly on mbsyncrc file template. Keeping on space
     line as following, or will failed to receive mail !
#+caption: Error information not keeping space line on mbsyncrc template
#+begin_example
$ mbsync -c ~/.emacs.d/.mbsyncrc -a
.emacs.d/.mbsyncrc:13: unknown keyword 'MaildirStore'
.emacs.d/.mbsyncrc:15: unknown keyword 'Inbox'
.emacs.d/.mbsyncrc:19: unknown store 'qq-local'
channel 'qq' refers to no slave store
#+end_example

#+caption: Correctly keeping space line as the code sample.
#+begin_example
IMAPAccount qq
Host imap.qq.com
...
<--- must be have at least one space line!!!
IMAPStore qq-remote
Account qq
...
Path ~/MyEmail/qq/
Inbox ~/MyEmail/qq/INBOX

<--- must be have at least one space line!!!
Channel qq
Master :qq-remote:
Slave :qq-local:
Patterns "INBOX" "Other" "Trash" "Archive" "Drafts" "Deleted" "Send"
Create Slave
Sync All
Expunge Both
SyncState *
...
*** in master
   ...
   3. ~git config --global user.email '2585957571@qq.com'~
...
* Spacemacs
  :CLOCKLOG:
  CLOCK: [2020-02-21 Fri 08:02]--[2020-02-21 Fri 09:32] =>  1:30
  :END:
** Configure my owen layer
*** Reference

1. [[https://medium.com/@cgrinaldi/creating-a-spacemacs-layer-from-an-existing-emacs-package-e][Creating a Spacemacs Layer from an Existing Emacs Package]]
...
#+begin_src emacs-lisp
(setq org-noter-layer-packages '(org-noter))
(defun org-noter-layer/init-org-noter ()
  (use-package org-noter
               :defer t))
#+end_src
* 参考
\printbibliography[title=参考文献]
1. :dvd: [[http://kitchingroup.cheme.cmu.edu/blog/category/org-mode/][The Kitchin Research Group org-mode]] 
